...
public class BaseDataObject implements Serializable, Cloneable, Remote, IBaseDataObject {
    ...
    /**
     * Utility class to serialize/deserialize to/from Kryo
     */
    public static class KryoSerializer extends Serializer<BaseDataObject> {
        ...
        public KryoSerializer(ClassResolver clzResolver) {
        ...
        public void write(Kryo kryo, Output output, BaseDataObject t) {
            ...
            output.writeString(t.workBundleId);
        ...
        public BaseDataObject read(Kryo kryo, Input input, Class<? extends BaseDataObject> type) {
            ...
            bdo.id = input.readString();
            bdo.workBundleId = input.readString();
    ...
    /**
     * Utility class to deserialize from a serialzed Kryo stream/file.
     */
    public static class DeserialzerIterator implements Iterator<List<IBaseDataObject>>, AutoCloseable {
        ...
        public DeserialzerIterator(InputStream src) {
            this.src = src;
            input = new Input(src, BUFF_SIZE);
            // initialize the iterator
            next = getNext();
        }

        public DeserialzerIterator(Path path) throws IOException {
        ...
        private ArrayList<IBaseDataObject> getNext() {
            return input.end() ? null : k.readObject(input, ArrayList.class);
        }
    }
}
